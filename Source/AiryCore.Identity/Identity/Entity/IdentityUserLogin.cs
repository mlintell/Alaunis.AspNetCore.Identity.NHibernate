...
namespace AiryCore.Identity.Entity
{
    ...
    /// <summary>
    /// The default login, for users that have a string based key.
    /// </summary>
    ...
    /// </summary>
    /// <typeparam name="TUser">The type of the user associated with this login.</typeparam>
    public class IdentityUserLogin<TUser> : IIdentityUserLogin<TUser>
    {
        /// <summary>
        ...
        protected bool ValueEquals(object other)
        {
            ...
            if (this.LoginProvider == obj.LoginProvider
                && this.ProviderDisplayName == obj.ProviderDisplayName
        ...
        public override int GetHashCode()
        {
            ...
            unchecked
            {
                ...
                int providerDisplayNameHash = this.ProviderDisplayName?.GetHashCode() ?? 0;
                int providerKeyHash = this.ProviderKey?.GetHashCode() ?? 0;
                int userHash = this.User == null ? 0 : this.User.GetHashCode();
                int hash = 17;

                hash = (hash * PrimeToMinimiseCollisions) + loginProviderHash;
                hash = (hash * PrimeToMinimiseCollisions) + providerDisplayNameHash;
